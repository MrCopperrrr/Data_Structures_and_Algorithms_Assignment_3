...
//////////////////////////////////////////////////////////////////////
    ...
    virtual float weight(T from, T to){
        ...
        if (fromNode == nullptr) {
            throw VertexNotFoundException(std::string(1, from));
        }

        VertexNode* toNode = getVertexNode(to);
        if (toNode == nullptr) {
            throw VertexNotFoundException(std::string(1, to));
        }

        // Tìm cạnh nối từ `from` đến `to`
        Edge* edge = fromNode->getEdge(getVertexNode(to));
        if (edge == nullptr) {
            throw EdgeNotFoundException("E(" + std::string(1, from) + "," + std::string(1, to) + ")");
        }
    ...
    virtual DLinkedList<T> getInwardEdges(T to){
        ...
        while (nodeIt != nodeList.end()) {
            VertexNode* currentNode = *nodeIt;

            // Duyệt qua danh sách các cạnh kề của currentNode
            ...
            while (edgeIt != currentNode->adList.end()) {
                ...
                if (vertexEQ(edge->to->vertex, toNode->vertex)) {
                    inwardEdges.add(currentNode->vertex); // Thêm đỉnh nguồn vào danh sách
                }
        ...
        return inwardEdges; // Trả về danh sách các đỉnh có cạnh đi vào `to`
    }
    ...
    virtual int inDegree(T vertex){
        ...
        if(node == nullptr) throw VertexNotFoundException(std::string(1, vertex)); // Use 'vertex' directly
    ...
    virtual bool connected(T from, T to){
        ...
        if(fromNode == nullptr) throw VertexNotFoundException(std::string(1, from)); // Use 'from' directly

        VertexNode* toNode = this->getVertexNode(to);
        if(toNode == nullptr) throw VertexNotFoundException(std::string(1, to)); // Use 'to' directly
