...
xMap<K,V>::xMap(const xMap<K,V>& map){
    ...
    // this->table=new DLinkedList<Entry*>(*(map.table));
    copyMapFrom(map);
}
...
V xMap<K,V>::remove(K key,void (*deleteKeyInMap)(K)){
    ...
    for (auto temp:list){
        if (keyEQ(temp->key,key)){
            V retValue=temp->value;
            if (deleteKeyInMap)
                deleteKeyInMap(temp->key);
            list.removeItem(temp,xMap<K,V>::deleteEntry);
            this->count--;
            return retValue;
        }
    }
...
bool xMap<K,V>::remove(K key, V value, void (*deleteKeyInMap)(K), void (*deleteValueInMap)(V)){
    ...
    DLinkedList<Entry*>& list=this->table[index];
    for (auto temp:list){
        if (keyEQ(temp->key,key)&&valueEQ(temp->value,value)){
            if (deleteKeyInMap)
                deleteKeyInMap(key);
            if (deleteValueInMap)
...
bool xMap<K,V>::containsValue(V value){
    ...
    for (int idx = 0; idx < capacity; idx++){
        for (auto temp:this->table[idx])
            if (valueEQ(temp->value,value))
                return 1;
    }
    return 0;
}
...
DLinkedList<V> xMap<K,V>::values(){
    //YOUR CODE IS HERE 
    DLinkedList<V> retValue;
    for (int idx = 0; idx < capacity; idx++){
        for (auto temp:this->table[idx])
            retValue.add(temp->value);
    }
    return retValue;
}
...
DLinkedList<int> xMap<K,V>::clashes(){
    //YOUR CODE IS HERE 
    DLinkedList<int> retclash;
    for (int idx = 0; idx < capacity; idx++)
        retclash.add(this->table[idx].size());
    return retclash;
}
