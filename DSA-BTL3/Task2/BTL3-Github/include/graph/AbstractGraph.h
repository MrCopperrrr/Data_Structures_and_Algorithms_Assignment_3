...
//////////////////////////////////////////////////////////////////////
    ...
    virtual void add(T vertex) {
        //TODO
        if (contains(vertex)) return; // Đỉnh đã tồn tại
    ...
    virtual bool contains(T vertex){
        //TODO
         typename DLinkedList<VertexNode*>::Iterator it = nodeList.begin();
    while (it != nodeList.end()) {
        if (vertexEQ((*it)->vertex, vertex)) return true;
        ++it;
    }
    return false;
    }
    ...
    while (nodeIt != nodeList.end()) {
        ...
        while (edgeIt != currentNode->adList.end()) {
            if ((*edgeIt)->to == toNode) {
                inwardEdges.add(currentNode->vertex);
            }
            ++edgeIt;
        }
    ...
    virtual void clear(){
        //TODO
         typename DLinkedList<VertexNode*>::Iterator it = nodeList.begin();
    while (it != nodeList.end()) {
        delete *it; // Xóa từng VertexNode
        ++it;
    }
    nodeList.clear(); // Xóa danh sách
    }
