...
public:
    ...
    void connect(T from, T to, float weight = 0)
    {
        ...
        if (fromNode == nullptr) {
            throw VertexNotFoundException("Vertex doesn't exist");  // Ném ngoại lệ nếu không tìm thấy đỉnh from
        }
        
        typename AbstractGraph<T>::VertexNode* toNode = this->getVertexNode(to);
        if (toNode == nullptr) {
            throw VertexNotFoundException("Vertex doesn't exist");  // Ném ngoại lệ nếu không tìm thấy đỉnh to
        }
    ...
    void disconnect(T from, T to)
    {
        ...
        if (from == to) {
            ...
            if (edge == nullptr) {
                throw EdgeNotFoundException("Edge doesn't exist");  // Ném ngoại lệ nếu không có cạnh
            }
    ...
    void remove(T vertex)
    {
        ...
        if (nodeToRemove == nullptr) {
            throw VertexNotFoundException("Vertex doesn't exist");  // Ném ngoại lệ nếu không tìm thấy đỉnh
        }
